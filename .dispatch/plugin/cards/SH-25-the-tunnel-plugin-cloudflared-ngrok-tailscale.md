...
4. **ngrok**: `PROGRAM = "ngrok"`; args `["http", listen]` plus `["--url", reserved]` when set;
...
6. **Build and list**: each plugin `cargo build --release --target wasm32-wasip2 -p tunnel-<provider>`; `sha256sum` of each `.wasm`;
